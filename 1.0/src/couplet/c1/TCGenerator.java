...
import couplet.c1.ic.Add;
import couplet.c1.ic.Arithmetic;
import couplet.c1.ic.BinaryArithmetic;
import couplet.c1.ic.CJump;
import couplet.c1.ic.Goto;
import couplet.c1.ic.ICVisitor;
import couplet.c1.ic.In;
import couplet.c1.ic.Instruction;
import couplet.c1.ic.InstructionList;
import couplet.c1.ic.IntegerLiteral;
import couplet.c1.ic.IntermediateCode;
import couplet.c1.ic.JEQ;
import couplet.c1.ic.JGE;
import couplet.c1.ic.JGT;
import couplet.c1.ic.JLE;
import couplet.c1.ic.JLT;
import couplet.c1.ic.JNE;
import couplet.c1.ic.Mov;
import couplet.c1.ic.Mul;
import couplet.c1.ic.Neg;
import couplet.c1.ic.Out;
import couplet.c1.ic.Sub;
import couplet.c1.ic.Temp;
import couplet.c1.ic.UnaryArithmetic;
import couplet.c1.ic.Variable;
...
import couplet.c1.x86.Location;
import couplet.c1.x86.MOV;
import couplet.c1.x86.Memory;
import couplet.c1.x86.NEG;
import couplet.c1.x86.Operand;
 ...
 * @author Goodness
 */
public class TCGenerator implements couplet.TCGenerator, ICVisitor {
	...
	@Override
	public void setIC(couplet.IntermediateCode ic) {
	...
	@Override
	public void visit(couplet.c1.ic.Label ic) throws Exception {
	...
	private couplet.c1.x86.Location checkSymbolTable(String name) {
	 ...
	 * @return 相应的目标代码操作数
	 */
	private Operand getTCOperand(couplet.c1.ic.Operand operand) {
		if (operand.getICType() == IntermediateCode.INTEGER_LITERAL) {
			...
			couplet.c1.x86.Location nowLocation = checkSymbolTable(operandName);
	...
	public void visit(In ic) throws Exception {
		couplet.c1.x86.Location dst = (couplet.c1.x86.Location) getTCOperand(ic
				.getDst());
	...
	public void visit(JNE ic) throws Exception {
		addCmp(ic);
		Label target = new Label(ic.getTarget().toString());
		codeSegment.add(new couplet.c1.x86.JNE(target));
	}
	...
	public void visit(JLE ic) throws Exception {
		addCmp(ic);
		Label target = new Label(ic.getTarget().toString());
		codeSegment.add(new couplet.c1.x86.JLE(target));
	}
	...
	public void visit(JGE ic) throws Exception {
		addCmp(ic);
		Label target = new Label(ic.getTarget().toString());
		codeSegment.add(new couplet.c1.x86.JGE(target));
	}
	 ...
	 */
	private void freeReg(couplet.c1.ic.Operand icOperand) {
	...
	private void addBinaryArithmetic(BinaryArithmetic ic, int flag) {
		couplet.c1.ic.Operand icSrc1 = ic.getSrc1();
		couplet.c1.ic.Operand icSrc2 = ic.getSrc2();
		couplet.c1.ic.Operand res = ic.getResult();
		...
		if (flag == 1) {
			...
			couplet.c1.x86.Location getDst = (couplet.c1.x86.Location) getTCOperand(ic
					.getResult());
	...
	private void addUnaryArithmetic(UnaryArithmetic ic, int flag) {
		couplet.c1.ic.Operand icSrc = ic.getSrc();
		couplet.c1.ic.Operand res = ic.getResult();
		if (flag == 1) {
